...
function toCommentRow(v: unknown): CommentRow {
  if (!isRecord(v)) {
    throw new Error("Invalid comment row");
  }

  const profiles = isRecord(v.profiles)
    ? ({ full_name: (v.profiles.full_name as string | null) ?? null } as ProfileMini)
    : null;
